...
const messagesWithRJ = messages.map((msg) => ({
...
async function handleDelete() {
  const duplicatesToDelete = scanResults.value.filter(
    (msg) => msg.status === "delete",
  );
  // ... 直接删除所有
}
```
  ...
  for (let i = 0; i < sortedMessages.length; i++) {
    ...
    for (let j = i + 1; j < sortedMessages.length; j++) {
      ...
      if (
        ...
        !usedBotMessages.has(nextMsg.messageId)
      ) {
        botReply = nextMsg;
...
async function deleteSelectedMessages() {
  const selectedMessages = scanResults.value.filter((row) =>
