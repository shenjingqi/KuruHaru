...
// Use getAsmrClient() from httpClient for proxy support
...
export async function loginStep1(params) {
  ...
  try {
    ...
    if (data.user && data.user.loggedIn) {
      ...
      if (cookies) {
        const tokenCookie = cookies.find(
          (c) => c.includes("token") || c.includes("jwt"),
        );
        if (tokenCookie) {
    ...
    const error = normalizeError(new Error("No token in response"), {
      context: "asmr_login",
    });
    return {
      success: false,
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          if (retryCount >= maxRetries) {
            logger.error(
              `第 ${pageNum} 页第 ${maxRetries} 次重试后仍失败:`,
              error.message,
            );
...
export async function triggerCloudDataFetch() {
  ...
  const result = await fetchCloudWorksAsync(
    config.asmr.token,
...
export async function loginAsmr(sender, params) {
  ...
  logger.info(
    `[ASMR-LOGIN] pathsToSave chineseListPath:`,
    pathsToSave.chineseListPath,
  );
  logger.info(
    `[ASMR-LOGIN] Object.keys(pathsToSave).length:`,
    Object.keys(pathsToSave).length,
  );
...
export async function checkAsmrLoginStatus() {
  try {
    ...
    Object.keys(config.paths || {}).forEach((key) => {
      if (
        config.paths[key] &&
        typeof config.paths[key] === "string" &&
        config.paths[key].trim()
      ) {
