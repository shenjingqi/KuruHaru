 ...
 * @returns {Promise<Object>}
 */
export async function login_(sender, params) {
  // 第一步：登录获取 token
  ...
  saveConfig({
    ...
    paths: currentConfig.paths, // 保留现有路径配置
  });
 ...
 */
export async function checkLoginStatus_() {
  try {
    const config = await getConfig();
...
export function setupAsmrIPCHandlers() {
  ...
  ipcMain.handle("asmr-login", async (event, params) => {
    return await login_(event.sender, params);
  });
