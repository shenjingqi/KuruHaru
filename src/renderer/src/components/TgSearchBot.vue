        ...
        <div class="usage-section">
          <n-card title="使用说明" size="small" :bordered="false">
            <ul class="usage-list">
              ...
              <li>Bot 会先在 Telegram 频道中搜索匹配的消息</li>
              <li>如未找到，会检查 Downloads/前置包.txt 文件</li>
              <li>如都未找到，会返回提示信息</li>
            </ul>
          </n-card>
        </div>
...
const getBotStatus = async () => {
  try {
    const result = await window.api.invoke("tg-bot-status");
...
const handleStartBot = async () => {
  ...
  try {
    const result = await window.api.invoke("tg-bot-start");
    if (result.success) {
...
const handleStopBot = async () => {
  ...
  try {
    const result = await window.api.invoke("tg-bot-stop");
    if (result.success) {
...
const handleSearch = async () => {
  ...
  try {
    const result = await window.api.invoke("tg-bot-search", rjCode);
